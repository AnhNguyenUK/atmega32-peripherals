...
#include "glcd.h"
unsigned int max_x = MAX_X;
unsigned int max_y = MAX_Y;
...
void glcd_init(void)
{
	...
	glcd_sendData(ILI9341_CMD_DISP_INVERSION_ON);
...
void glcd_char(unsigned char ascii, unsigned int x0, unsigned int y0, unsigned int size, unsigned int fgcolor)	
{
	...
	for (i =0; i<FONT_X; i++ )
	{
		...
		{
			if((temp>>f)&0x01)
				glcd_fillRectangle(x0+offsetx-i*size, y0+offsety+f*size, size, size, fgcolor);
		}
	}
}
